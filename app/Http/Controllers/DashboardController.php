    ...
    public function getKpiJsonData() {
        ...
        if (sizeof($uids) > 0) {
            ...
            $team_result = User::select(DB::raw('(select count(bd.id) from bd where user_id in ('.$uids.') and deleted_at is null and date >= "'.$sdate.'" and date <= "'.$edate.'") as bd_count,
                ...
                (select count(hunt_face.id) from hunt_face where created_by in ('.$uids.') and type = "二面" and deleted_at is null and date >= "'.$sdate.'" and date <= "'.$edate.'" and (select count(hunt_records.id) from hunt_records where hunt_records.hunt_id = hunt_face.hunt_id) > 0) as faces_count,
        ...
        $person_result = User::select(DB::raw('(select count(bd.id) from bd where user_id = users.id and deleted_at is null and date >= "'.$sdate.'" and date <= "'.$edate.'") as bd_count,
            ...
            (select count(hunt_face.id) from hunt_face where created_by = users.id and type = "二面" and deleted_at is null and date >= "'.$sdate.'" and date <= "'.$edate.'" and (select count(hunt_records.id) from hunt_records where hunt_records.hunt_id = hunt_face.hunt_id) > 0) as faces_count,
