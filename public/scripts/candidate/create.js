...
$(function () {
    ...
    $('#degree, [name="salary"]').kendoDropDownList();
    ...
    $.$ajax.get('/data/location.json', function (res) {
        res.province.unshift({id: 0, name: '-选择省份-', citys: []});
        $('#location-province').kendoDropDownList({
            dataSource: res.province,
            dataTextField: 'name',
            dataValueField: 'name',
            filter: 'startswith',
            change: function (e) {
                var province = this.dataItem(), citys = province.citys, cityData = [];
                $.each(citys, function (i, cid) {
                    cityData.push(res.citys[cid]);
                });
                $('#location-city').data('kendoDropDownList').setDataSource(cityData);
            }
        });
        $('#belong-province').kendoDropDownList({
            dataSource: res.province,
            dataTextField: 'name',
            dataValueField: 'name',
            change: function (e) {
                ...
                $('#belong-city').data('kendoDropDownList').setDataSource(cityData);
            }
        });
        $('#location-city, #belong-city').kendoDropDownList({
            dataSource: [{id: 0, name: '-选择城市-'}],
            dataTextField: 'name',
            dataValueField: 'name',
            filter: 'startswith'
        });
    ...
    //$.$ajax.get('/data/company.json', function (res) {
    //    console.log(res);
        $('[name="company"]').kendoAutoComplete({
            dataSource: {
                transport: {
                    read: {
