            ...
            <div class="row">
                <form name="form" class="form form-horizontal" onsubmit="return false" novalidate>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-xs-10">
                            <label class="form-control-static orange">{{$bd->name}}</label>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="control-label col-xs-2"><span class="red">*</span> 企业名称</label>
                        <div class="col-xs-10">
                            <label class="form-control-static">{{$bd->company_name}}</label>
                        </div>
                    </div>
                    <div class="form-group">
                        <div class="col-xs-6 no-padding error-box">
                            ...
                            <div class="col-xs-8">
                                <label class="form-control-static">{{$bd->user}}</label>
                            </div>
                        </div>
                        <div class="col-xs-6 no-padding error-box">
                            <label class="control-label col-xs-4"><span class="red">*</span> 接入日期</label>
                            <div class="col-xs-8">
                                <label class="form-control-static">{{$bd->date}}</label>
                            </div>
                        </div>
                    </div>
                    <div class="form-group">
                        <div class="col-xs-6 no-padding error-box">
                            ...
                            <div class="col-xs-8">
                                <label class="form-control-static">{{$bd->type}}</label>
                            </div>
                        </div>
                        <div class="col-xs-6 no-padding error-box">
                            <label class="control-label col-xs-4">状态</label>
                            <div class="col-xs-8">
                                <label class="form-control-static">{{$bd->status}}</label>
                            </div>
                        </div>
                    </div>
                    <div class="form-group">
...
@section('body-script')
        ...
        $(function () {
            ...
            $('#grid').kendoGrid({
                ...
                columns: [
                    ...
                    {title: '操作', template: '<a href="/bd/download-file/?path=#:path#&name=#:path.split(\'/\').pop()#" class="btn btn-info btn-sm" title="下载文件"><i class="fa fa-download"></i></a>' +
                    ...
                    buttons: [
                        {
                            ...
                            handler: function () {
                                ...
                                if (filePath == '') return;
                                $.$ajax.post('/bd/add-file', {bd_id: bd_id, filePath: filePath, fileDesc: fileDesc}, function (res) {
                                    if (~~res) {
                                        self.hide();
    ...
    <script>
        (function () {
            ...
            var bd_id = {{$bd->id}}, sessionId = {{Session::get('id')}}, sessionNickname = '{{Session::get('nickname')}}';

            app.controller('bdController', ['$scope', '$http', function ($scope, $http) {
                $scope.records = [];

                $http.get('/bd/record-list?bd_id='+bd_id).success(function (res) {
                    $scope.records = res;
                });
